...
var (
	...
	myTitleStyle = lipgloss.NewStyle().
			Padding(1, 10).
			Foreground(lipgloss.Color("#FFF7DB")).
...
func RenderTitleStyle(s string) string {
	return myTitleStyle.Render(s)
}
...
func RenderMap(m map[string]interface{}) string {
	...
	for _, k := range keys {
		returnString += "\"" + keyStyle.Render(k) + "\""
		returnString += " : "
		returnString += valStyle.Render(fmt.Sprint(m[k]))
		returnString += ",\n"
	}
...
func RenderLayout() {
	...
	// Title
	{
		...
		for i, v := range colors {
			...
			fmt.Fprint(&title, titleStyle.SetString("Sai Jonnala").Copy().MarginLeft(i*offset).Background(c))
			if i < len(colors)-1 {
				title.WriteRune('\n')
			}
		}
